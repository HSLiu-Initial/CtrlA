...
class RepControlPipeline(TextGenerationPipeline):
    def __init__(self,
                 ...
                 **kwargs):
        assert control_method == "reading_vec", f"{
            control_method} not supported yet"
        assert block_name == "decoder_block" or "LlamaForCausalLM" in model.config.architectures, f"{
            model.config.architectures} {block_name} not supported yet"
    ...
    def __call__(self, text_inputs, activations=None, **kwargs):
        if activations is not None:
            ...
            self.wrapped_model.set_controller(
                self.layers, activations, self.block_name)
